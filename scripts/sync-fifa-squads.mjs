...
function serializeCatalog(entries) {
  ...
  const imagePrefix = 'https://digitalhub.fifa.com/transform/';
  const imageSuffix = '?io=transform:fill,aspectratio:1x1,width:320,gravity:top&quality=75';
  const squadPrefix =
    'https://www.fifa.com/pt/tournaments/mens/worldcup/canadamexicousa2026/teams/';
  const squadSuffix = '/squad';
  const sourceLabel = 'Elenco atualizado a partir da lista oficial da FIFA.';
  const serialized = JSON.stringify(catalog, null, 2)
    .replace(/"imageUrl": "([^"]+)"/g, (match, url) =>
      url.startsWith(imagePrefix) && url.endsWith(imageSuffix)
        ? `"imageUrl": fifaImage(${JSON.stringify(url.slice(imagePrefix.length, -imageSuffix.length))})`
        : match,
    )
    .replace(/"sourceUrl": "([^"]+)"/g, (match, url) =>
      url.startsWith(squadPrefix) && url.endsWith(squadSuffix)
        ? `"sourceUrl": fifaSquad(${JSON.stringify(url.slice(squadPrefix.length, -squadSuffix.length))})`
        : match,
    )
    .replaceAll(
...
export const teamCatalogByCode: Record<string, TeamCatalogEntry> = ${serialized};
`;
}
...
async function main() {
  ...
  process.stdout.write(
    `Catalogo salvo com ${entries.length} selecoes e ${totalPlayers} jogadores.\n`,
  );
}
