...
describe('componentes obrigatórios de release', () => {
  it('explica ranking, movimento, distância e desempate sem critério oculto', () => {
    render(
      <RankingTable
        ...
        previousRanks={new Map([['current', 4]])}
  ...
  it('mantém feedback de erro recuperável e estados SSE inequívocos', () => {
    ...
    const { rerender } = render(
      <AsyncState status="error" error="Falha fixture" onRetry={retry} />,
    );
  ...
  it('anuncia, substitui e permite fechar feedback toast', async () => {
    function Harness() {
      const toast = useToast();
      return (
        <Pressable
          accessibilityRole="button"
          onPress={() => toast.showToast('Salvo com fixture', 'success')}
        >
          <Text>Mostrar aviso</Text>
        </Pressable>
      );
    }
    render(
    ...
    await waitFor(() => expect(screen.queryByText('Salvo com fixture')).toBeNull());
  });

  it('exige classificado válido no empate do mata-mata', () => {
    const participants = { homeTeamId: 'home', awayTeamId: 'away' };
    expect(
      resolvedAdvancingTeam({ home: '1', away: '1', advancingTeamId: null }, participants),
    ).toBeNull();
    expect(
      resolvedAdvancingTeam({ home: '1', away: '1', advancingTeamId: 'other' }, participants),
    ).toBeNull();
    expect(
      resolvedAdvancingTeam({ home: '1', away: '1', advancingTeamId: 'away' }, participants),
    ).toBe('away');
    expect(
      resolvedAdvancingTeam({ home: '2', away: '1', advancingTeamId: null }, participants),
    ).toBe('home');
  });
