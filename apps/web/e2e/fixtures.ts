...
const worldSeason = {
  ...
  capabilities: { workspace: 'WORLD_CUP_LEGACY', groupStage: true, knockoutBracket: true, liveScoring: true },
};
const leagueSeason = {
...
const hybridSeason = {
  id: 'season-hybrid', competitionId: 'competition-hybrid', slug: 'torneio-hibrido-2026', name: 'Torneio Híbrido 2026', year: 2026,
  timezone: 'America/Sao_Paulo', status: 'ACTIVE', startsAt: '2026-07-01T00:00:00.000Z', endsAt: '2026-12-20T00:00:00.000Z',
  capabilities: { format: 'GROUPS', groupStage: true, standings: true, knockout: true, twoLegs: true, rankingScopes: ['OVERALL', 'ROUND'] },
};
...
const hybridRound = {
  id: 'round-hybrid-1', seasonId: hybridSeason.id, stageId: hybridStage.id, name: 'Rodada 1', order: 1, status: 'ACTIVE',
  startsAt: '2026-07-16T22:30:00.000Z', endsAt: '2026-07-23T22:30:00.000Z', stage: hybridStage,
};
const genericMatch = {
...
export async function installApiMocks(page: Page, options: { authenticated?: boolean; loginStatus?: number; admin?: boolean; closed?: boolean } = {}) {
  ...
  await page.route('**/api/**', async (route: Route) => {
    ...
    if (path === '/api/competitions') return json({ competitions: [
      { id: 'competition-world', slug: 'world-cup', name: 'Copa do Mundo', capabilities: { workspace: 'WORLD_CUP_LEGACY', groupStage: true, knockoutBracket: true, liveScoring: true } },
      ...
      { id: 'competition-hybrid', slug: 'torneio-hibrido', name: 'Torneio Híbrido', capabilities: { format: 'GROUPS', groupStage: true, standings: true, knockout: true, twoLegs: true, rankingScopes: ['OVERALL', 'ROUND'] } },
    ], pagination: { ...pagination, total: 3 } });
    if (path.includes('/api/competitions/world-cup/seasons')) return json({ competition: { id: 'competition-world', slug: 'world-cup', name: 'Copa do Mundo', capabilities: { workspace: 'WORLD_CUP_LEGACY', groupStage: true, knockoutBracket: true, liveScoring: true } }, seasons: [worldSeason], pagination });
    ...
    if (path.includes('/api/competitions/torneio-hibrido/seasons')) return json({ competition: { id: 'competition-hybrid', slug: 'torneio-hibrido', name: 'Torneio Híbrido', capabilities: { format: 'GROUPS', groupStage: true, standings: true, knockout: true, twoLegs: true, rankingScopes: ['OVERALL', 'ROUND'] } }, seasons: [hybridSeason], pagination });
    ...
    if (path === `/api/seasons/${hybridSeason.id}/rounds`) return json({ rounds: [hybridRound], pagination });
    if (path === `/api/seasons/${hybridSeason.id}/matches`) return json({ matches: [hybridMatch], pagination });
    if (path === `/api/seasons/${hybridSeason.id}/standings`) return json({ standingsByGroup: [{ group: 'Grupo A', rows: [standing(1, brazil, 3), standing(2, argentina, 0)] }], pagination: { ...pagination, total: 2 } });
    if (path === `/api/seasons/${hybridSeason.id}/teams`) return json({ teams: hybridTeamSummaries, pagination: { ...pagination, total: 2 } });
    if (path.includes(`/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/predictions`) && method === 'GET') return json({ predictions: [], pagination: { ...pagination, total: 0, totalPages: 0 } });
    if (path === `/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/rules`) return json({
      scoring: { id: 'rules-hybrid', key: 'hybrid', name: 'Pontuação híbrida', version: 1, rules: { exactScore: 15, correctOutcome: 3, oneTeamGoals: 1, miss: 0 } },
      tieBreakers: { id: 'tie-hybrid', key: 'hybrid', name: 'Desempate híbrido', version: 1, allowSharedPositions: false, criteria: [{ field: 'exactScores', direction: 'desc', label: 'Placares exatos' }] },
      predictionPolicy: { scoreableFrom: null, scoreableFromRound: null, startsAtRound: null, historicalMatchesScoreable: false },
    });
    if (path === `/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/engagement`) return json({
      achievements: [], streaks: [], notifications: [],
      preferences: { inAppEnabled: true, pushEnabled: false, emailEnabled: false, quietHoursEnabled: false, quietHoursStart: null, quietHoursEnd: null, timezone: 'America/Sao_Paulo' },
    });
    if (path.includes(`/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/ranking`) && method === 'GET') return json({ ranking, pagination: { ...pagination, total: 2 } });
    if (path === `/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/ranking/visit` && method === 'POST') return json({ summary: { previousRank: null, currentRank: 2, delta: null } });
    if (path === `/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/awards`) return json({ awards: [] });
    if (path === `/api/pools/${POOL_SLUG}/seasons/${hybridSeason.id}/sync-status`) return json({ providers: [], lastSyncedAt: null, syncing: false });
