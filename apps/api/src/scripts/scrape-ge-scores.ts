...
} from '../services/score-sync.logic.js';
import {
...
async function applyKnockoutScore(score: ScrapedScore) {
  ...
  const winnerTeam = score.winnerTeam
    ? teams.find((team) => normalizeName(team.name) === normalizeName(score.winnerTeam!))
...
async function runOnce() {
  ...
  try {
    ...
    logger.info(
      { scraped: scores.length, results, topScorers: topScorers?.length ?? null },
      'GE scrape finished',
    );
  } catch (error) {
    await prisma.apiSyncLog.create({
