...
function decodeHtml(value: string) {
  return value
    .replace(/\\u([\dA-Fa-f]{4})/g, (_match, code: string) => String.fromCharCode(parseInt(code, 16)))
    .replace(/&amp;/g, '&')
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
    .replace(/&apos;/g, "'")
...
function statusFromGe(value?: string) {
  ...
  if (['encerrado', 'encerrada', 'fimdejogo', 'finalizado', 'finalizada'].some((text) => normalized.includes(text))) {
    return MatchStatus.FINISHED;
  }
  if (['andamento', 'aovivo', 'intervalo', 'tempo'].some((text) => normalized.includes(text))) return MatchStatus.LIVE;
...
async function scrapeGeScores() {
  ...
  for (const url of GE_URLS) {
    ...
    for (const matchObject of matchObjects) {
      ...
      if (!score) continue;
      scores.set(`${normalizeName(score.homeTeam)}:${normalizeName(score.awayTeam)}:${score.homeScore}:${score.awayScore}`, {
        ...score,
...
function parseTopScorersFromHtml(html: string) {
  ...
  return section
    ...
    .map((item): ScrapedTopScorer | null => {
      ...
      const teamFlagUrl = firstMatch(item, /<div class="jogador-escudo">[\s\S]*?<img\s+src="([^"]+)"/i);
      const teamName = firstMatch(item, /<div class="jogador-escudo">[\s\S]*?<img[^>]+alt="([^"]+)"/i);
...
async function applyScore(score: ScrapedScore) {
  ...
  const match = candidates.find(
    (candidate) => normalizeName(candidate.homeTeam.name) === home && normalizeName(candidate.awayTeam.name) === away,
  );

  if (!match) return { updated: false, reason: 'MATCH_NOT_FOUND', score };
  ...
  const data: Prisma.MatchUpdateInput = {
    ...
    rawPayload: {
      ...(match.rawPayload && typeof match.rawPayload === 'object' && !Array.isArray(match.rawPayload)
        ? match.rawPayload
        : {}),
      geScore: {
  ...
  try {
    ...
    await prisma.apiSyncLog.create({
      data: {
        ...
        finishedAt: new Date(),
        details: JSON.parse(
          JSON.stringify({ scraped: scores.length, results, topScorers: topScorers?.length ?? null }),
        ) as Prisma.InputJsonValue,
      },
    });

    logger.info({ scraped: scores.length, results, topScorers: topScorers?.length ?? null }, 'GE scrape finished');
  } catch (error) {
    await prisma.apiSyncLog.create({
      data: {
        source: 'ge.globo.com:copa',
...
async function main() {
  ...
  setInterval(() => {
    runOnce().catch((error) => logger.error({ error }, 'GE score scrape failed'));
  }, POLL_MS);
}
