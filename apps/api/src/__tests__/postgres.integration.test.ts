...
const { Pool } = pg;
const pool = new Pool({ connectionString: process.env.DATABASE_URL, application_name: 'stage9-integration' });

async function storeCall<T>(operation: (callback: (error?: unknown, value?: T | null) => void) => void) {
...
describe('PostgreSQL real: sessão e CSRF', () => {
  it('persiste, lê, revoga a sessão e aplica CSRF usando connect-pg-simple', async () => {
    ...
    await expect(storeCall<session.SessionData>((callback) => resources.store.get(sid, callback))).resolves.toMatchObject({ csrfToken: value.csrfToken });
    await storeCall<void>((callback) => resources.store.destroy(sid, callback));
    await expect(storeCall<session.SessionData>((callback) => resources.store.get(sid, callback))).resolves.toBeUndefined();
...
describe('PostgreSQL real: concorrência, constraints e isolamento', () => {
  it('serializa palpites concorrentes antes do limite e falha fechado no instante exato', async () => {
    ...
    const count = await pool.query('SELECT count(*)::int AS count FROM "Prediction" WHERE "userId" = $1 AND "matchId" = $2', ['user-stage9', 'match-stage9-cup']);
    expect(count.rows[0].count).toBe(1);

    vi.setSystemTime(new Date('2030-01-01T12:00:00.000Z'));
    await expect(upsertPredictions('day-stage9-cup', 'user-stage9', input)).rejects.toMatchObject({ code: 'PREDICTION_MATCH_CLOSED', statusCode: 409 });
  });

  it('rejeita relação cruzada e mantém unicidade composta no banco', async () => {
    await expect(pool.query(`INSERT INTO "Prediction" (id, "userId", "matchId", "poolSeasonId", "predictedHomeScore", "predictedAwayScore", "updatedAt") VALUES ('prediction-cross-scope', 'user-stage9-2', 'match-stage9-other', 'pool-season-bolao-do-trabalho-world-cup-2026', 1, 0, now())`)).rejects.toMatchObject({ code: '23514' });
    await expect(pool.query(`INSERT INTO "PoolMembership" (id, "poolId", "userId", role, status, "updatedAt") VALUES ('membership-duplicate', 'pool-bolao-do-trabalho', 'user-stage9', 'MEMBER', 'ACTIVE', now())`)).rejects.toMatchObject({ code: '23505' });
  });
});
...
describe('PostgreSQL real: import, override, outbox e ranking', () => {
  it('torna import concorrente idempotente e rollback de override auditável', async () => {
    const insert = () => pool.query(`INSERT INTO "ProviderSyncRun" (id, provider, "seasonId", type, "idempotencyKey", "dryRun", status, source) VALUES ($1, 'fixture', 'competition-season-world-cup-2026', 'SCHEDULE', 'stage9-import-key', false, 'SUCCESS', 'local-fixture') ON CONFLICT (provider, "seasonId", type, "idempotencyKey") DO NOTHING`, [randomUUID()]);
    await Promise.all([insert(), insert(), insert()]);
    const imports = await pool.query('SELECT count(*)::int AS count FROM "ProviderSyncRun" WHERE "idempotencyKey" = $1', ['stage9-import-key']);
    expect(imports.rows[0].count).toBe(1);

    await pool.query(`INSERT INTO "MatchOverride" (id, "matchId", "actorId", justification, values, before, active, "updatedAt") VALUES ('override-stage9', 'match-stage9-cup', 'user-stage9', 'Fixture local de rollback', '{"status":"FINISHED"}', '{"status":"SCHEDULED"}', true, now())`);
    await pool.query('UPDATE "MatchOverride" SET active = false, "removedAt" = now(), "updatedAt" = now() WHERE id = $1', ['override-stage9']);
    const override = await pool.query('SELECT active, before, values FROM "MatchOverride" WHERE id = $1', ['override-stage9']);
    expect(override.rows[0]).toMatchObject({ active: false, before: { status: 'SCHEDULED' }, values: { status: 'FINISHED' } });
  });
  ...
  it('publica outbox somente após commit e deduplica replay', async () => {
    ...
    try {
      ...
      await client.query(`INSERT INTO "OutboxEvent" (id, type, "seasonId", "poolSeasonId", payload, "idempotencyKey") VALUES ('outbox-rolled-back', 'fixture.event', 'competition-season-world-cup-2026', 'pool-season-bolao-do-trabalho-world-cup-2026', '{}', 'outbox-rollback')`);
      await client.query('ROLLBACK');
    } finally {
      client.release();
    }
    const rolledBack = await pool.query('SELECT count(*)::int AS count FROM "OutboxEvent" WHERE id = $1', ['outbox-rolled-back']);
    expect(rolledBack.rows[0].count).toBe(0);

    await Promise.all([1, 2].map((value) => pool.query(`INSERT INTO "OutboxEvent" (id, type, "seasonId", "poolSeasonId", payload, "idempotencyKey") VALUES ($1, 'fixture.event', 'competition-season-world-cup-2026', 'pool-season-bolao-do-trabalho-world-cup-2026', $2, 'outbox-commit') ON CONFLICT ("idempotencyKey") DO NOTHING`, [`outbox-${value}`, JSON.stringify({ value })])));
    const committed = await pool.query('SELECT count(*)::int AS count FROM "OutboxEvent" WHERE "idempotencyKey" = $1', ['outbox-commit']);
  ...
  it('isola snapshots de ranking e mantém histórico anterior ao início não pontuável', async () => {
    ...
    const scoped = await pool.query('SELECT sum(points)::int AS points FROM "RankingSnapshot" WHERE "poolSeasonId" = $1', ['pool-season-bolao-do-trabalho-world-cup-2026']);
    const gate = await pool.query('SELECT "startsAtRound", "scoreableFromRound", "historicalMatchesScoreable" FROM "PoolSeason" WHERE id = $1', ['pool-season-stage9-other']);
    expect(scoped.rows[0].points).toBe(12);
    expect(gate.rows[0]).toMatchObject({ startsAtRound: 2, scoreableFromRound: 2, historicalMatchesScoreable: false });
  });
});
